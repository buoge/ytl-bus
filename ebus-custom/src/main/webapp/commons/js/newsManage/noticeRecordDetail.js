...
setTimeout(function(){
	 var valueStatus= $('#noticeRange').val();
	 selectVal(valueStatus);
},100)
function selectVal(val){
	  if(val==3){
		  $("#specifyScope").show();
		  $("#chooseRouter").hide();
		  $('#anoticeRange').combobox('setValue',val);
		  $('#anoticeRange').combobox('setText',"指定地点范围");
		  //根据地址地图初始化
		  getCity();
	  }
	  else if(val==4){
		  ...
		  $("#specifyScope").hide();
		  $('#anoticeRange').combobox('setValue',val);
		  $('#anoticeRange').combobox('setText',"指定线路范围");
		  routerList();
	  }
	  else if(val==1 || val==2){
		  $("#chooseRouter").hide();
		  ...
		  if(val==2){
			  $('#anoticeRange').combobox('setValue',val);
			  $('#anoticeRange').combobox('setText',"全城");  
		  }
		  else{
			  $('#anoticeRange').combobox('setValue',val);
	...
	$('#routeStr').combobox({
		...
		loadFilter:function(data){
			if(data.success) {
				...
				routerStrName=data.obj[0].routename+',';
				routerStrId=data.obj[0].routeid+',';
...
function mapInit(cityNames){
	var points=rowpoints;
	//判断是否存在城市 无则依据ip定位地图
	if(!cityNames){	
		return
	}
	//判断是否存在 标记值存在则标记 并且以第一个标记为中心点
	if(points.length>0){
		mapObj = new AMap.Map('iCenter', {
	         level: 11,//地图显示的比例尺级别
	         center: [points[0].latitude,points[0].longitude],//地图中心点
	         resizeEnable: true
	     });
		 //初始化坐标点
		 setPoint();
   ...
   AMap.event.addDomListener(document.getElementById('deleteMarker'), 'click', function() {
 	for(var i=0;i<marker.length;i++){
 		...
 		for(var j=0;j<markerAreaBj.length;j++){
 			if(markerData.I==markerArea[j].I&&markerData.M==markerAreaBj[j].M&&markerData.lng==markerAreaBj[j].lng&&markerData.lat==markerAreaBj[j].lat){
...
function setPoint(){
	...
	for(var i=0;i<points.length;i++){
		 var markerDetial=new AMap.Marker({                    
			 icon:appName+'/commons/images/marker.png',
			 position:new AMap.LngLat(points[i].latitude,points[i].longitude)  
	});  
		 markerDetial.setMap(mapObj);  //在地图上添加点  
	}
}
