...
public class GotoCarAlertSetting extends Model{

    /**
     * 
     */
    private static final long serialVersionUID = 1800902469093734741L;
    ...
    public GotoCarAlertSetting( Integer userId, String cityCode, Integer tag, Integer alertStrategy, Boolean isRemind,
	    Boolean isOpen, String startTime, String endTime) {
	super();
	this.userId = userId;
    ...
    /**
     * 功能描述:根据jsonObj构建GotoCarAlertSetting对象
     * 作者:温海金
     * 最后更改时间 : 2017年3月6日 下午5:58:57
     */
    public static GotoCarAlertSetting buid(JSONObject obj) {
	Integer userId=null, alertStrategy=null, tag=null;
	    if(!StringUtils.isEmpty(obj.get("userId"))){
		userId = Integer.valueOf(obj.get("userId").toString());
	    }
	    if(!StringUtils.isEmpty(obj.get("alertStrategy"))){
		alertStrategy = Integer.valueOf(obj.get("alertStrategy").toString());
	    }
	    String cityCode = obj.get("cityCode").toString();
	    String endTime = obj.get("endTime").toString();
	    Boolean isOpen=false;
	    if(!StringUtils.isEmpty(obj.get("isOpen"))){
		isOpen =  Boolean.valueOf(obj.get("isOpen").toString());
	    }
	    String startTime = obj.get("startTime").toString();
	    if(!StringUtils.isEmpty(obj.get("tag"))){
		tag =  Integer.valueOf(obj.get("tag").toString());
	    }

	    return new GotoCarAlertSetting(userId, cityCode, tag, alertStrategy, false, isOpen, startTime, endTime);
	    
    }

    public GotoCarAlertSetting(String id, Integer userId, String cityCode, Integer tag, Integer alertStrategy,
	    Boolean isRemind, Boolean isOpen, String startTime, String endTime, Date createTime) {
	super();
	this.id = id;
	this.userId = userId;
	this.cityCode = cityCode;
